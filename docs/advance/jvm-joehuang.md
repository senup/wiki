...
1. **-Xms**：设置堆内存的初始大小。也就是在 JVM 启动时分配的内存大小，比如-Xms256m 就表示分配 256MB 的内存。
...
1. **明确优化目标**：首先，我们需要明确优化的目标，例如：降低响应时间、提高吞吐量、减少 OOM 出现的频率等等。
...
CPU 飙高和内存飙高，它们之间就像是一队舞蹈团队中的两位舞者，虽然各自有各自的动作，但它们之间可能存在着某种联系。
