...
<span class="line"><span style="color:#24292E;">&lt;/</span><span style="color:#22863A;">dependency</span><span style="color:#24292E;">&gt;</span></span></code></pre></div><p>其中注意SpringBoot是作为父工程项目引入的，web模块是作为依赖引入的。</p><p>然后我们需要手动创建如下文件：</p><p>其中<code>SpringBoothelloWorldApplication.java</code>中代码如下：</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#F97583;">package</span><span style="color:#E1E4E8;"> cn.bestzuo.springboot.helloworld;</span></span>
<span class="line"></span>
...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><p>这个主类是启动SpringBoot的主程序类，也是SpringBoot应用启动的入口，当然这个类内部做了什么事我们先不管，我们继续编写Controller类来实现HelloWorld。</p><h2 id="实现helloworld" tabindex="-1">实现HelloWorld <a class="header-anchor" href="#实现helloworld" aria-label="Permalink to &quot;实现HelloWorld&quot;">​</a></h2><p>创建类<code>HelloWorld</code>如下：</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#F97583;">package</span><span style="color:#E1E4E8;"> cn.bestzuo.springboot.helloworld.controller;</span></span>
<span class="line"></span>
...
<span class="line"><span style="color:#24292E;">&lt;/</span><span style="color:#22863A;">parent</span><span style="color:#24292E;">&gt;</span></span>
<span class="line"><span style="color:#24292E;">他来真正管理Spring Boot应用里面的所有依赖版本；</span></span></code></pre></div><p>对了，这个SpringBoot的版本号就相当于是SpringBoot的版本仲裁中心，内部解决了所有Spring可能用到的依赖造成的冲突，所以在SpringBoot项目中导入依赖默认是不需要写版本号的，当然了，没有在dependencies里面管理的依赖自然需要声明版本号。</p><p>然后我们再来看一看web启动模块：</p><div class="language-xml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">xml</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">&lt;</span><span style="color:#85E89D;">dependency</span><span style="color:#E1E4E8;">&gt;</span></span>
<span class="line"><span style="color:#E1E4E8;">    &lt;</span><span style="color:#85E89D;">groupId</span><span style="color:#E1E4E8;">&gt;org.springframework.boot&lt;/</span><span style="color:#85E89D;">groupId</span><span style="color:#E1E4E8;">&gt;</span></span>
<span class="line"><span style="color:#E1E4E8;">    &lt;</span><span style="color:#85E89D;">artifactId</span><span style="color:#E1E4E8;">&gt;spring-boot-starter-web&lt;/</span><span style="color:#85E89D;">artifactId</span><span style="color:#E1E4E8;">&gt;</span></span>
<span class="line"><span style="color:#E1E4E8;">&lt;/</span><span style="color:#85E89D;">dependency</span><span style="color:#E1E4E8;">&gt;</span></span></code></pre><pre class="shiki github-light vp-code-light"><code><span class="line"><span style="color:#24292E;">&lt;</span><span style="color:#22863A;">dependency</span><span style="color:#24292E;">&gt;</span></span>
<span class="line"><span style="color:#24292E;">    &lt;</span><span style="color:#22863A;">groupId</span><span style="color:#24292E;">&gt;org.springframework.boot&lt;/</span><span style="color:#22863A;">groupId</span><span style="color:#24292E;">&gt;</span></span>
<span class="line"><span style="color:#24292E;">    &lt;</span><span style="color:#22863A;">artifactId</span><span style="color:#24292E;">&gt;spring-boot-starter-web&lt;/</span><span style="color:#22863A;">artifactId</span><span style="color:#24292E;">&gt;</span></span>
<span class="line"><span style="color:#24292E;">&lt;/</span><span style="color:#22863A;">dependency</span><span style="color:#24292E;">&gt;</span></span></code></pre></div><p>我们可以打开这个依赖包看一下：</p><p>然后就发现了这个玩意的本质，原来在这个依赖包中，已经默认帮我们导入了Spring的web和webmvc模块，并且还有相关的jackson依赖以及日志包等等，甚至还有一个神奇的spring-boot-starter-tomcat启动器依赖，这就不难理解为什么上面我们只导入了一个依赖却能轻松的使用这么多组件。原来都是SpringBoot帮我们一站式解决了这些依赖。</p><p>而细心的人应该已经发现上面还有一个<code>Spring-boot-starter</code>，翻译过来就是SpringBoot启动器，这个依赖就是SpringBoot的核心依赖，Spring Boot将所有的功能场景都抽取出来，做成一个个的starters（启动器），只需要在项目里面引入这些starter相关场景的所有依赖都会导入进来。要用什么功能就导入什么场景的启动器 。</p><h2 id="主程序类" tabindex="-1">主程序类 <a class="header-anchor" href="#主程序类" aria-label="Permalink to &quot;主程序类&quot;">​</a></h2><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">@</span><span style="color:#F97583;">SpringBootApplication</span></span>
<span class="line"><span style="color:#F97583;">public</span><span style="color:#E1E4E8;"> </span><span style="color:#F97583;">class</span><span style="color:#E1E4E8;"> </span><span style="color:#B392F0;">SpringbootHelloworldApplication</span><span style="color:#E1E4E8;"> {</span></span>
<span class="line"><span style="color:#E1E4E8;">    </span><span style="color:#F97583;">public</span><span style="color:#E1E4E8;"> </span><span style="color:#F97583;">static</span><span style="color:#E1E4E8;"> </span><span style="color:#F97583;">void</span><span style="color:#E1E4E8;"> </span><span style="color:#B392F0;">main</span><span style="color:#E1E4E8;">(</span><span style="color:#F97583;">String</span><span style="color:#E1E4E8;">[] </span><span style="color:#FFAB70;">args</span><span style="color:#E1E4E8;">) {</span></span>
<span class="line"><span style="color:#E1E4E8;">        SpringApplication.</span><span style="color:#B392F0;">run</span><span style="color:#E1E4E8;">(SpringbootHelloworldApplication.class, args);</span></span>
<span class="line"><span style="color:#E1E4E8;">    }</span></span>
...
<span class="line"><span style="color:#24292E;">        SpringApplication.</span><span style="color:#6F42C1;">run</span><span style="color:#24292E;">(SpringbootHelloworldApplication.class, args);</span></span>
<span class="line"><span style="color:#24292E;">    }</span></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><p>然后回到主程序类中，我们首先会关注@SpringBootApplication这个注解，这个注解的外在含义比较容易理解，就是Spring Boot应用标注在某个类上从而说明这个类是SpringBoot的主配置类，SpringBoot就可以运行这个类的main方法来启动SpringBoot应用。</p><p>那么这个注解的内在含义呢？我们可以点开这个注解类一探究竟。</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">@</span><span style="color:#F97583;">Target</span><span style="color:#E1E4E8;">(ElementType.TYPE)</span></span>
...
<span class="line"><span style="color:#22863A;">spring</span><span style="color:#24292E;">:</span></span>
<span class="line"><span style="color:#24292E;">	</span><span style="color:#22863A;">profiles</span><span style="color:#24292E;">: </span><span style="color:#032F62;">prod</span><span style="color:#24292E;"> </span><span style="color:#6A737D;">#指定属于哪个环境</span></span></code></pre></div><p>那么如何切换呢？</p><ul><li>在配置文件中指定 spring.profiles.active=dev</li><li>命令行： java -jar spring-boot-02-confifig-0.0.1-SNAPSHOT.jar --spring.profifiles.active=dev； 可以直接在测试的时候，配置传入命令行参数</li><li>虚拟机参数； -Dspring.profifiles.active=dev</li></ul><p>当然实际中用的最多的还是第一种方式，直接指定即可。</p>`,86),r=[l];function e(t,i,c,E,g,u){return n(),a("div",null,r)}const d=s(p,[["render",e]]);export{f as __pageData,d as default};
